...
export default class Handler implements IHandler {
    ...
    async LoadCommands() {
        ...
        files.map(async (file:string) => {
            ...
            if (file.split("/").pop()?.split(".")[2]) {
                return this.client.subCommands.set(command.name, command as SubCommand);
            }
    ...
    async LoadTasks() {
        const files = (await glob(`build/tasks/**/*.js`)).map(filePath => path.resolve(filePath));

        files.map(async (file:string) => {
            const task = new (await import(file)).default(this.client);
            if (!task.name) {
                return delete require.cache[require.resolve(file)] && console.log(`${file.split("/").pop()} does not have a name.`);
            };
            this.client.tasks.set(task.name, task);
